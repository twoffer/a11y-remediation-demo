...
- **Forced colours**: a Windows contrast theme on `index.html` and `compare.html?f=form-labels`. Checked that focus rings, the skip link, the toggle's two states, the pane, diff box, and table edges, and iframe text survive the repaint.
- **Serving**: every page at `http://localhost:8000/`. The same pass at the GitHub Pages URL runs after the first deploy, and is the one group of rows still open in the record.
